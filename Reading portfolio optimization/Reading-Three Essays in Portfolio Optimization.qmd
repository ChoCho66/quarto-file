...
這些模型中最著名的當然是 Markowitz (1952) 開發的均值-方差模型。
...
在第 1 章中，我們討論了成功的投資組合優化模型的選擇，並開發了一個結合了 Markowitz（1952）的全局最小方差（GMV）投資組合和另一個著名模型的優勢的模型，
...
因此，我們只考慮資產之間預期收益的相對值。
我們的模型對經驗數據集的數值應用表明，基於該模型的投資組合在復合年回報率和夏普比率方面明顯優於 GMV 投資組合和 1/N 投資組合。
...
符號預測可以應用於交易策略。
在這些策略中，
投資於股票市場或更安全的替代品——例如債券或國庫券——基於下一個時期從股票市場獲得正回報的可能性。
...
0%、-2%、-4%、-6% 和-8%。
作為解釋變量，我們使用通過 GARCH(1,1) 方法計算的條件波動率，
無條件偏度，
...
我們特別分析了 RP 模型在人壽保險行業的應用和優勢，
由於 Solvency II 指令，它面臨著巨大的變化。
該指令——歐盟的新監管框架——制定了新的資本要求，
估值技術，
保險公司的管理和報告標準。
它包括數量要求，
這迫使保險公司計算其負債的“公允”價值。
...
在第 3 章中，我們重點關注 RP 優化過程的第二步和第三步。
